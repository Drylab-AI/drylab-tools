...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const r = await fetch(`${backend}/jobs/${params.id}`, { method: 'GET', cache: 'no-store' })
    if (!r.ok) return NextResponse.json({ error: 'Upstream error' }, { status: r.status, headers: { 'Cache-Control': 'no-store' } })
    const data = await r.json()
    return NextResponse.json(data, { headers: { 'Cache-Control': 'no-store' } })
  } catch (e) {
    return NextResponse.json({ error: 'Failed to fetch job' }, { status: 502, headers: { 'Cache-Control': 'no-store' } })
  }
}
